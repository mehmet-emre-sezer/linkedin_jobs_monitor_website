...
import { MOCK_USERS, MOCK_USER_DETAILS } from "@/constants/mockData"
...
export default async function UserDetailPage({ params }: Props) {
  ...
  return (
    <div className="p-8 space-y-6 max-w-4xl">
      {/* Geri linki */}
      <Link href="/admin/users" className="text-xs text-gray-500 hover:text-gray-300 transition-colors inline-flex items-center gap-1">
        ← Kullanıcılar
      </Link>
      ...
      <div className="flex items-center gap-4">
        ...
        <span className={`text-xs px-3 py-1 rounded-full border ${
          ...
          {user.subscription === "paid" ? "Ücretli" : "Ücretsiz"}
        </span>
      </div>
        ...
        <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-5">
          ...
          <div className="space-y-3 text-sm">
            <div>
              ...
              <div className={user.isTelegramConnected ? "text-blue-400" : "text-red-400"}>
                {user.isTelegramConnected ? "● Bağlı" : "● Bağlı değil"}
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="text-gray-300 tabular-nums">{formatDate(user.registeredAt)}</div>
            </div>
            <div>
              <div className="text-xs text-gray-600 mb-0.5">Son giriş</div>
              <div className="text-gray-300 tabular-nums">{formatDate(user.lastSeenAt)}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div>
            ...
            <div className="text-2xl font-bold text-white tabular-nums">{detail?.totalJobsScanned ?? 0}</div>
          </div>
          <div>
            <div className="text-xs text-gray-600 mb-1">Gönderilen</div>
            <div className="text-2xl font-bold text-blue-400 tabular-nums">{detail?.totalJobsSent ?? 0}</div>
          </div>
          <div>
            <div className="text-xs text-gray-600 mb-1">Ortalama skor</div>
            <div className="text-2xl font-bold text-green-400 tabular-nums">{detail?.averageScore ?? 0}</div>
          </div>
        </div>
      </div>
    </div>
